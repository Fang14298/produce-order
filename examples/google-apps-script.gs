...
// ฟังก์ชั่นสร้าง EMVCo PromptPay Payload String แบบออฟไลน์
function generatePromptPayPayload(target, amount) {
  target = String(target || "").replace(/[^0-9]/g, '');
  if (target.length === 10 && target.startsWith('0')) {
    target = '0066' + target.substring(1);
  }
  var targetLength = String(target.length).padStart(2, '0');
  var targetField = '01' + targetLength + target;
  var subFields = '0016A000000677010111' + targetField;
  var subFieldsLength = String(subFields.length).padStart(2, '0');
  var merchantField = '29' + subFieldsLength + subFields;

  var amountStr = amount ? Number(amount).toFixed(2) : '';
  var amountField = '';
  if (amountStr) {
    var amountLength = String(amountStr.length).padStart(2, '0');
    amountField = '54' + amountLength + amountStr;
  }

  var rawPayload = '000201010212' + merchantField + '5303764' + amountField + '5802TH6304';
