...
* **หน้าพิมพ์ใบส่งสินค้า/ใบแจ้งหนี้ (แอดมิน)**: [`invoice.html`](https://fang14298.github.io/produce-order/invoice.html)
...
* **หน้าที่**: บันทึกประวัติและรายละเอียดการสั่งซื้อทั้งหมด (สร้างชีตและจัดสไตล์หัวตารางอัตโนมัติหากยังไม่มี) — เป็นแหล่งข้อมูลเดียวกับที่ [`invoice.html`](#-ระบบพิมพ์ใบส่งสินค้า--ใบแจ้งหนี้สำหรับแอดมิน-invoicehtml) ใช้ดึงมาออกบิล
...
## 🧾 ระบบพิมพ์ใบส่งสินค้า / ใบแจ้งหนี้สำหรับแอดมิน (`invoice.html`)

เว็บเพจแยกต่างหากจาก `index.html` (ลูกค้าไม่ต้องเห็น) สำหรับให้แอดมินค้นหาออเดอร์ที่มีอยู่แล้วในชีต `"รายการออเดอร์"` แล้วออกเป็นใบส่งสินค้า/ใบแจ้งหนี้ขนาด A4 พร้อมสั่งพิมพ์หรือดาวน์โหลดเป็น PDF

* **อ่านข้อมูลสด ไม่ต้องมี backend เพิ่ม**: ดึงออเดอร์ทั้งหมดจากชีต `"รายการออเดอร์"` ผ่าน Google Visualization API (`gviz/tq`) แบบเดียวกับที่ `index.html` ใช้ดึงราคาจากชีต `"ราคาระบบ"` — ไม่ต้องแก้ `google-apps-script.gs` หรือเปิด endpoint ใหม่ใดๆ (อ่านอย่างเดียว, ปลอดภัย) ข้อกำหนดเดียวคือชีตต้องแชร์แบบ *"ทุกคนที่มีลิงก์ - ผู้มีสิทธิ์ดู"* เหมือนเดิม
* **ค้นหา + เลือกออเดอร์**: มีช่องค้นหาด้วยเลขที่ออเดอร์ / ชื่อลูกค้า / เบอร์โทร กดเลือกจากรายการด้านซ้ายเพื่อขึ้นบิลด้านขวาทันที (ออเดอร์ล่าสุดจะอยู่บนสุดของรายการเหมือนในชีต)
* **แปลงข้อความสรุปกลับเป็นตารางรายชิ้น**: อ่านค่าคอลัมน์ H (`รายการสินค้าที่สั่ง`) ที่เก็บเป็นข้อความสรุป แล้วแยกกลับเป็นแถวสินค้า (ชื่อ/จำนวน/หน่วย/ราคาต่อหน่วย/จำนวนเงิน) ด้วย regex — ราคาต่อหน่วยคำนวณจาก `จำนวนเงิน ÷ จำนวน` เนื่องจากชีตยังไม่ได้เก็บราคาต่อหน่วยแยกไว้
* **PromptPay Dynamic QR Code ล็อกยอด**: สร้าง QR ตามมาตรฐาน EMVCo ด้วยไลบรารี `qrcode-generator` (ออฟไลน์ ไม่พึ่ง API ภายนอก) ล็อกยอดเงิน = ยอดรวมออเดอร์ + ค่าจัดส่ง (ปรับได้ที่ช่องกรอกด้านบน เพราะชีตยังไม่ได้แยกเก็บค่าส่ง) ไปยัง PromptPay ID `093-3121105`
* **ปุ่มพิมพ์บิล**: `window.print()` พร้อม CSS `@page { size: A4 }` ซ่อน UI ที่ไม่เกี่ยวข้องตอนพิมพ์ เหลือแค่ตัวบิล
* **ปุ่มดาวน์โหลด PDF**: แปลงตัวบิลเป็นภาพด้วย `html2canvas` แล้วสร้างไฟล์ PDF ขนาด A4 ด้วย `jsPDF` ดาวน์โหลดเป็น `ใบแจ้งหนี้-<เลขที่ออเดอร์>.pdf` ทันที ไม่ต้องผ่านหน้าต่างพิมพ์ของเบราว์เซอร์
* **ดีไซน์บิล**: จัดตามแบบที่ออกแบบไว้ (สไตล์ Makro) — โลโก้ร้าน + ข้อมูลติดต่อ, ข้อมูลลูกค้า, ตารางรายการสินค้า, กล่อง QR ชำระเงิน, สรุปยอด, ช่องเซ็นชื่อผู้จัดส่ง/ผู้รับสินค้า

> ⚠️ **ข้อควรทราบ**: ค่าจัดส่งยังไม่มีคอลัมน์เก็บแยกในชีต (README ค่าเริ่มต้นคือส่งฟรีในระยะ 10 กม.) หน้า `invoice.html` จึงมีช่องกรอกค่าจัดส่งแบบแมนวลให้แอดมินปรับก่อนพิมพ์/ดาวน์โหลดทุกครั้ง
...
├── invoice.html                     # หน้าพิมพ์ใบส่งสินค้า/ใบแจ้งหนี้สำหรับแอดมิน (ค้นหาออเดอร์ + PromptPay QR + พิมพ์/ดาวน์โหลด PDF)
