...
* **ระบบพิมพ์ใบส่งสินค้า/ใบแจ้งหนี้ (แอดมิน)**: แยกเป็นโปรเจกต์ต่างหาก → [`produce-order-invoice`](https://github.com/Fang14298/produce-order-invoice) — เว็บใช้งานจริงที่ [https://fang14298.github.io/produce-order-invoice/](https://fang14298.github.io/produce-order-invoice/)
...
* **หน้าที่**: บันทึกประวัติและรายละเอียดการสั่งซื้อทั้งหมด (สร้างชีตและจัดสไตล์หัวตารางอัตโนมัติหากยังไม่มี) — เป็นแหล่งข้อมูลเดียวกับที่ระบบพิมพ์ใบส่งสินค้า/ใบแจ้งหนี้ (ดูหัวข้อถัดไป) ใช้ดึงมาออกบิล
* **โครงสร้างคอลัมน์ (A-L)**:
  ...
  | D | เบอร์โทรศัพท์ | `0812345678` |
  | E | ที่อยู่จัดส่ง | `123/45 ถ.สุขุมวิท แขวงคลองเตย เขตคลองเตย กทม. 10110` |
  | F | เลขผู้เสียภาษี | `0105551234567` (หรือ `-`) |
  | G | วันที่รับของ | `2026-08-11` |
  | H | รายการสินค้าที่สั่ง | `• คะน้า 2 กก. = 80 บาท\n• มะนาว 10 ลูก = 30 บาท` |
  | I | ยอดรวม(บาท) | `110` |
  | J | หมายเหตุ | `ขอมะเขือเทศลูกแข็ง` |
  | K | สถานะ | `รอยืนยัน` |
  | L | LINE User ID | `U1234567890abcdef...` |
...
## 🧾 ระบบพิมพ์ใบส่งสินค้า / ใบแจ้งหนี้สำหรับแอดมิน (แยกเป็นโปรเจกต์ต่างหาก)

ย้ายออกจาก repo นี้แล้ว — ตอนนี้อยู่ที่โปรเจกต์แยกต่างหาก [`produce-order-invoice`](https://github.com/Fang14298/produce-order-invoice) เพื่อไม่ให้ปนกับโค้ดหน้าเว็บลูกค้า และ deploy/แก้ไขได้อิสระจากกัน

* **เว็บใช้งานจริง**: [https://fang14298.github.io/produce-order-invoice/](https://fang14298.github.io/produce-order-invoice/)
* **หน้าที่**: แอดมินค้นหาออเดอร์จากชีต `"รายการออเดอร์"` (ชีตเดียวกับ repo นี้) แล้วออกเป็นใบส่งสินค้า/ใบแจ้งหนี้ขนาด A4 พร้อม PromptPay QR ล็อกยอด สั่งพิมพ์ หรือดาวน์โหลดเป็น PDF
* **สถาปัตยกรรม**: อ่านข้อมูลสดผ่าน Google Visualization API (`gviz/tq`) เหมือนหน้าเว็บลูกค้า — อ่านอย่างเดียว ไม่ต้องแก้ `google-apps-script.gs` หรือเปิด endpoint ใหม่ใดๆ ข้อกำหนดเดียวคือชีตต้องแชร์แบบ *"ทุกคนที่มีลิงก์ - ผู้มีสิทธิ์ดู"* เหมือนเดิม

ดูรายละเอียดฟีเจอร์ทั้งหมดได้ที่ README ของ [`produce-order-invoice`](https://github.com/Fang14298/produce-order-invoice#readme)
...
> 📦 ระบบพิมพ์ใบส่งสินค้า/ใบแจ้งหนี้ (เดิมคือ `invoice.html`) ย้ายไปอยู่ repo แยก [`produce-order-invoice`](https://github.com/Fang14298/produce-order-invoice) แล้ว ไม่ได้อยู่ใน repo นี้อีกต่อไป
