...
### 2. การทำงานฝั่งร้านค้าและแอดมิน (Admin Experience)
* **ระบบบันทึกออเดอร์รวมศูนย์**: บันทึกออเดอร์ลงตาราง `"รายการออเดอร์"` ใน Google Sheet พร้อมสร้าง Order ID ในรูปแบบ `ORD-YYYYMMDD-HHmmss` อัตโนมัติ
...
> หากต้องการทดสอบรันสคริปต์ในหน้า Apps Script ให้เลือกฟังก์ชั่น **`testDoPost`** แล้วกด **"เรียกใช้ (Run)"** (ห้ามเลือก `doPost` โดยตรง เพราะ `doPost(e)` รับค่าพารามิเตอร์ `e.postData` จากคำขอ HTTP Webhook ของหน้าเว็บ หากกดรัน `doPost` โดยตรงจะขึ้นแจ้งเตือน `Cannot read properties of undefined (reading 'postData')`)
